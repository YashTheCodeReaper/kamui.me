...
export class ScrollerComponent implements AfterViewInit {
  ...
  currentScroll = 0;
  resizeTimeout: any;
  @ViewChild('portfolioScrollerSection')
  portfolioScrollerSectionEl!: ElementRef;
  ...
  initScene(): void {
    try {
      ...
      const camera = new THREE.PerspectiveCamera(
        45,
        this.portfolioScrollerSectionEl.nativeElement.offsetWidth /
          this.portfolioScrollerSectionEl.nativeElement.offsetHeight,
        0.1,
        1000
      );
      const renderer = new THREE.WebGLRenderer({
        canvas: document.querySelector(
          '#portfolioScrollerCanvas'
      ...
      const parentWidth = 20 / 1.5;
      const parentHeight = 75 / 1.5;
      const curvature = 35;
      const segmentsX = 200;
      const segmentsY = 200;
      ...
      parentMesh.rotation.x = THREE.MathUtils.degToRad(0);
      ...
      camera.lookAt(0, 0, 0);
      // camera.rotation.z = THREE.MathUtils.degToRad(-3);
